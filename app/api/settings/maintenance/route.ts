...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = 'SELECT `value`, end_time FROM settings WHERE `key` = ? LIMIT 1';
    const result = await executeQuery(query, ['maintenance_mode']);
    
    const maintenanceMode = result[0]?.value === 'true' || false;
    const endTime = result[0]?.end_time || null;
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 'key' kelimesi reserved keyword olduğu için backtick kullanıyoruz
    const updateQuery = `
      INSERT INTO settings (\`key\`, \`value\`, end_time) 
      VALUES ('maintenance_mode', ?, ?) 
      ON DUPLICATE KEY UPDATE \`value\` = VALUES(\`value\`), end_time = VALUES(end_time)
    `;
    
    await executeQuery(updateQuery, [maintenanceMode.toString(), endTime || null]);
